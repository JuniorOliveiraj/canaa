...
export default function BlogPostCard({ post, index, href }) {
  const { cover, title, view, comment, share, author, createdAt , id} = post;
  const linkTo = `${href === 'blog' ? '/blog':PATH_DASHBOARD.blog.root}/post/${paramCase(title)}-${id}${href !== 'blog' ? '?adm=true' :''}`;
