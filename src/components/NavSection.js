...
// material
import { alpha, useTheme, styled, Typography } from '@mui/material';
...
function NavItem({ item, active, isShow }) {
  ...
  if (children) {
    console.log( isShow ? true : false , info ? true : false )
    return (
      <>
        <ListItemStyle
          onClick={handleOpen}
          ...
          {isShow && (
            <>
              ...
              {info && info}
              <Iconify icon={'10k'} width={22} height={22} />
              <Typography> 
                
                
                <Iconify
               
               icon={open ? 'arrow-ios-downward-fill' : 'arrow-ios-forward-fill'}
