...
export default function CheckeTheme() {
  ...
  const [checked, setChecked] = useState(() => {
    ...
    return storedValue ? JSON.parse(storedValue) : darkModeThem;
  });
    ...
    axios.get(url, { headers })
      .then(response => {
        if (response.data.themastatus) {
          const darkMode = response.data.themastatus === '1';
          setDarkModeThem(darkMode);
          setChecked(darkMode);
        }
      })
      .catch(error => {
  ...
  }, [userId, token, checked, setDarkModeThem]);
  useEffect(() => {
  ...
  const switchHandler = (event) => {
    ...
    setDarkModeThem(event.target.checked);
  };
  return (
    <FormControlLabel
